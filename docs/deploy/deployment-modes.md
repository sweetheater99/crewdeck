...
Crewdeck supports two runtime modes with different security profiles.
...
```sh
pnpm crewdeck allowed-hostname my-machine
```
...
When migrating from `local_trusted` to `authenticated`, Crewdeck emits a one-time claim URL at startup:
...
```sh
CREWDECK_DEPLOYMENT_MODE=authenticated pnpm crewdeck run
```
