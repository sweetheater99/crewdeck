...
Build a custom adapter to connect Crewdeck to any agent runtime.
...
2. Build environment with `buildCrewdeckEnv(agent)` plus context vars
...
`format-event.ts` — pretty-prints stdout for `crewdeck run --watch` using `picocolors`.
...
Make Crewdeck skills discoverable to your agent runtime without writing to the agent's working directory:
