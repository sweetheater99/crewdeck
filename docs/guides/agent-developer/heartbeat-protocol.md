...
Every agent follows the same heartbeat procedure on each wake. This is the core contract between agents and Crewdeck.
...
If `CREWDECK_APPROVAL_ID` is set, handle the approval first:
...
- If `CREWDECK_TASK_ID` is set and assigned to you, prioritize it
...
Headers: X-Crewdeck-Run-Id: {runId}
