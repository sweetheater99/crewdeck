# ClipHub: Marketplace for Crewdeck Team Configurations

> The "app store" for whole-company AI teams — pre-built Crewdeck configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for Crewdeck-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | Crewdeck company (agents, projects, governance) |
...
A complete Crewdeck company configuration:
...
Single-agent configurations designed to plug into a Crewdeck org:
...
Portable skill files that any Crewdeck agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with Crewdeck's skill loading system
...
interface Listing {
  ...
  crewdeckVersionMin: string;     // Minimum Crewdeck version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target Crewdeck company
...
| `POST` | `/api/listings/:id/install` | Install to Crewdeck company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target Crewdeck company (or create new)
  → ClipHub API calls Crewdeck API to:
  ...
  → Redirect to Crewdeck dashboard with new team running
```
...
### 5.3 Creator: Export from Crewdeck → Publish

```
Running Crewdeck company → "Export as Blueprint" (CLI or UI)
  → Crewdeck exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (Crewdeck brand blue/purple)
...
| Install flow | One-click deploy to Crewdeck company |
...
{
  "targetCompanyId": "uuid",         // Existing Crewdeck company
...
3. For each agent in blueprint:
   - `POST /api/companies/:id/agents` (if `crewdeck-create-agent` supports it, or via approval flow)
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as Crewdeck
- **Backend**: Node.js API (or extend Crewdeck server)
- **Database**: Postgres (can share Crewdeck's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with Crewdeck auth (or OAuth2)

### 9.2 Integration with Crewdeck

ClipHub can be:
- **Option A**: A separate app that calls Crewdeck's API to install blueprints
- **Option B**: A built-in section of the Crewdeck UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing Crewdeck UI and API.
...
- [ ] Install flow: blueprint → Crewdeck company
...
- [ ] "Export from Crewdeck" CLI command
