...
Crewdeck exposes a RESTful JSON API for all control plane operations.
...
- **Agent run JWTs** — short-lived tokens injected during heartbeats (`CREWDECK_API_KEY`)
...
- Run audit trail: include `X-Crewdeck-Run-Id` header on all mutating requests during heartbeats
