...
describe("agent local JWT", () => {
  const secretEnv = "CREWDECK_AGENT_JWT_SECRET";
  const ttlEnv = "CREWDECK_AGENT_JWT_TTL_SECONDS";
  const issuerEnv = "CREWDECK_AGENT_JWT_ISSUER";
  const audienceEnv = "CREWDECK_AGENT_JWT_AUDIENCE";
  ...
  it("rejects issuer/audience mismatch", () => {
    ...
    process.env[issuerEnv] = "crewdeck";
    process.env[audienceEnv] = "crewdeck-api";
    expect(verifyLocalAgentJwt(token!)).toBeNull();
  });
});
