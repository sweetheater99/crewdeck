...
import { isCodexUnknownSessionError, parseCodexJsonl } from "@crewdeck/adapter-codex-local/server";
import { parseCodexStdoutLine } from "@crewdeck/adapter-codex-local/ui";
  ...
  it("parses turn and reasoning lifecycle events", () => {
    ...
    expect(
      parseCodexStdoutLine(
        JSON.stringify({
          type: "item.completed",
          item: { id: "item_1", type: "reasoning", text: "**Preparing to use crewdeck skill**" },
        }),
        ts,
      ),
    ).toEqual([
      { kind: "thinking", ts, text: "**Preparing to use crewdeck skill**" },
    ]);
  });
