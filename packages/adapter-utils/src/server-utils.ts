...
export function buildCrewdeckEnv(agent: { id: string; companyId: string }): Record<string, string> {
  ...
  const runtimeHost = resolveHostForUrl(
    process.env.CREWDECK_LISTEN_HOST ?? process.env.HOST ?? "localhost",
  );
  const runtimePort = process.env.CREWDECK_LISTEN_PORT ?? process.env.PORT ?? "3100";
  const apiUrl = process.env.CREWDECK_API_URL ?? `http://${runtimeHost}:${runtimePort}`;
  vars.CREWDECK_API_URL = apiUrl;
  return vars;
}
