...
Use when:
- You want Crewdeck to run Cursor Agent CLI locally as the agent runtime
...
- Crewdeck auto-injects local skills into "~/.cursor/skills" when missing, so Cursor can discover "$crewdeck" and related skills on local runs.
- Crewdeck auto-adds --yolo unless one of --trust/--yolo/-f is already present in extraArgs.
`;
