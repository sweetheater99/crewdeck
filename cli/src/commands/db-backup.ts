...
import { formatDatabaseBackupResult, runDatabaseBackup } from "@crewdeck/db";
import {
  expandHomePrefix,
...
function resolveConnectionString(configPath?: string): { value: string; source: string } {
  ...
  return {
    value: `postgres://crewdeck:crewdeck@127.0.0.1:${port}/crewdeck`,
...
export async function dbBackupCommand(opts: DbBackupOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" crewdeck db:backup ")));

  const configPath = resolveConfigPath(opts.config);
  const config = readConfig(opts.config);
  const connection = resolveConnectionString(opts.config);
  const defaultDir = resolveDefaultBackupDir(resolveCrewdeckInstanceId());
  ...
  const filenamePrefix = opts.filenamePrefix?.trim() || "crewdeck";
