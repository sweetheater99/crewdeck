...
import { ApiRequestError, CrewdeckApiClient } from "../../client/http.js";
...
export function addCommonClientOptions(command: Command, opts?: { includeCompany?: boolean }): Command {
  command
    .option("-c, --config <path>", "Path to Crewdeck config file")
    .option("-d, --data-dir <path>", "Crewdeck data directory root (isolates state from ~/.crewdeck)")
    .option("--context <path>", "Path to CLI context file")
    .option("--profile <name>", "CLI context profile name")
    .option("--api-base <url>", "Base URL for the Crewdeck API")
...
export function resolveCommandContext(
  ...
  const apiBase =
    ...
    process.env.CREWDECK_API_URL?.trim() ||
    profile.apiBase ||
    inferApiBaseFromConfig(options.config);

  const apiKey =
    options.apiKey?.trim() ||
    process.env.CREWDECK_API_KEY?.trim() ||
    readKeyFromProfileEnv(profile);

  const companyId =
    options.companyId?.trim() ||
    process.env.CREWDECK_COMPANY_ID?.trim() ||
    profile.companyId;
  ...
  const api = new CrewdeckApiClient({ apiBase, apiKey });
  return {
    api,
    companyId,
    profileName,
...
function inferApiBaseFromConfig(configPath?: string): string {
  const envHost = process.env.CREWDECK_SERVER_HOST?.trim() || "localhost";
  let port = Number(process.env.CREWDECK_SERVER_PORT || "");
