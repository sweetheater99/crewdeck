...
export async function runCommand(opts: RunOptions): Promise<void> {
  const instanceId = resolveCrewdeckInstanceId(opts.instance);
  process.env.CREWDECK_INSTANCE_ID = instanceId;

  const homeDir = resolveCrewdeckHomeDir();
  ...
  p.intro(pc.bgCyan(pc.black(" crewdeck run ")));
  ...
  if (!configExists(configPath)) {
    if (!process.stdin.isTTY || !process.stdout.isTTY) {
      ...
      p.log.message(`Run ${pc.cyan("crewdeck onboard")} once, then retry ${pc.cyan("crewdeck run")}.`);
      process.exit(1);
    }
  ...
  p.log.step("Starting Crewdeck server...");
...
function maybeEnableUiDevMiddleware(entrypoint: string): void {
  if (process.env.CREWDECK_UI_DEV_MIDDLEWARE !== undefined) return;
  const normalized = entrypoint.replaceAll("\\", "/");
  if (normalized.endsWith("/server/src/index.ts") || normalized.endsWith("@crewdeck/server/src/index.ts")) {
    process.env.CREWDECK_UI_DEV_MIDDLEWARE = "true";
  }
}
...
async function importServerEntry(): Promise<void> {
  ...
  // Production mode: import the published @crewdeck/server package
  try {
    await import("@crewdeck/server");
  } catch (err) {
    const missingSpecifier = getMissingModuleSpecifier(err);
    const missingServerEntrypoint = !missingSpecifier || missingSpecifier === "@crewdeck/server";
    if (isModuleNotFoundError(err) && missingServerEntrypoint) {
      throw new Error(
        `Could not locate a Crewdeck server entrypoint.\n` +
          `Tried: ${devEntry}, @crewdeck/server\n` +
          `${formatError(err)}`,
      );
    }
    throw new Error(
      `Crewdeck server failed to start.\n` +
        `${formatError(err)}`,
    );
  }
}
