...
const DATA_DIR_OPTION_HELP =
  "Crewdeck data directory root (isolates state from ~/.crewdeck)";

program
  .name("crewdeck")
  .description("Crewdeck CLI — setup, diagnose, and configure your instance")
  .version("0.2.7");
...
program
  ...
  .option("--run", "Start Crewdeck immediately after saving config", false)
  .action(onboard);

program
  .command("doctor")
  .description("Run diagnostic checks on your Crewdeck setup")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
program
  ...
  .option("--filename-prefix <prefix>", "Backup filename prefix", "crewdeck")
...
program
  .command("run")
  .description("Bootstrap local setup (onboard + doctor) and run Crewdeck")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
heartbeat
  ...
  .option("--api-base <url>", "Base URL for the Crewdeck server API")
