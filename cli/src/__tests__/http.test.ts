...
describe("CrewdeckApiClient", () => {
  ...
  it("adds authorization and run-id headers", async () => {
    ...
    const client = new CrewdeckApiClient({
    ...
    expect(headers["x-crewdeck-run-id"]).toBe("run-abc");
  ...
  it("returns null on ignoreNotFound", async () => {
    ...
    const client = new CrewdeckApiClient({ apiBase: "http://localhost:3100" });
