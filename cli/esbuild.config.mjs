/**
 * esbuild configuration for building the crewdeck CLI for npm.
 *
 * Bundles all workspace packages (@crewdeck/*) into a single file.
...
// to npm and resolved at runtime (e.g. @crewdeck/server uses dynamic import).
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@crewdeck/")) {
      externals.add(name);
    }
  }
