...
type CrewdeckSprite = (typeof CREWDECK_SPRITES)[number];
...
function spriteSize(sprite: CrewdeckSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = CREWDECK_SPRITES[(baseCol + baseRow) % CREWDECK_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = CREWDECK_SPRITES[Math.floor(Math.random() * CREWDECK_SPRITES.length)]!;
