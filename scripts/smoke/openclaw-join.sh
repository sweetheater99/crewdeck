...
CREWDECK_API_URL="${CREWDECK_API_URL:-http://localhost:3100}"
API_BASE="${CREWDECK_API_URL%/}/api"
COMPANY_ID="${COMPANY_ID:-${CREWDECK_COMPANY_ID:-}}"
...
SMOKE_IMAGE="${SMOKE_IMAGE:-crewdeck-openclaw-smoke:local}"
SMOKE_CONTAINER_NAME="${SMOKE_CONTAINER_NAME:-crewdeck-openclaw-smoke}"
SMOKE_PORT="${SMOKE_PORT:-19091}"
SMOKE_TIMEOUT_SEC="${SMOKE_TIMEOUT_SEC:-45}"

AUTH_HEADERS=()
if [[ -n "${CREWDECK_AUTH_HEADER:-}" ]]; then
  AUTH_HEADERS+=(-H "Authorization: ${CREWDECK_AUTH_HEADER}")
fi
if [[ -n "${CREWDECK_COOKIE:-}" ]]; then
  AUTH_HEADERS+=(-H "Cookie: ${CREWDECK_COOKIE}")
fi
...
Provide one of:
  CREWDECK_AUTH_HEADER=\"Bearer <board-token>\"
  CREWDECK_COOKIE=\"<board-session-cookie>\"
...
assert_status "200"
if ! grep -q "Crewdeck OpenClaw Onboarding" <<<"$RESPONSE_BODY"; then
...
for _ in $(seq 1 "$SMOKE_TIMEOUT_SEC"); do
  ...
  MATCH_COUNT="$(jq -r --arg agentId "$CREATED_AGENT_ID" '[.events[] | select(((.body.crewdeck.agentId // "") == $agentId))] | length' <<<"$LAST_EVENTS")"
