# Crewdeck Module System

## Overview

Crewdeck's module system lets you extend the control plane with new capabilities — revenue tracking, observability, notifications, dashboards — without forking core. Modules are self-contained packages that register routes, UI pages, database tables, and lifecycle hooks.
...
|---------|-----------|----------------|
| **Module** | A package that extends Crewdeck's API, UI, and data model | Yes |
...
modules/
  observability/
    crewdeck.module.json     # manifest (required)
    src/
...
- **`id`**: Unique identifier, used as the npm package name suffix (`@crewdeck/mod-observability`)
...
```typescript
import type { ModuleAPI } from "@crewdeck/core";
...
```
1. Scan modules/ directory for crewdeck.module.json manifests
...
```jsonc
// crewdeck.config.json (or env vars)
{
  "modules": {
...
export const modulePages = [
  {
    path: "/observability",
    label: "Observability",
    component: lazy(() => import("@crewdeck/mod-observability/ui")),
  },
];
...
export const dashboardWidgets = [
  {
    ...
    label: "Token Burn Rate",
    placement: "dashboard",
    component: lazy(() => import("@crewdeck/mod-observability/ui").then(m => ({ default: m.TokenBurnRateWidget }))),
  },
];
```
  ...
  "modules": [
    {
      ...
      "repo": "github:crewdeck/mod-observability",
      "version": "0.1.0",
  ...
  "templates": [
    {
      ...
      "url": "https://store.crewdeck.ing/templates/startup-in-a-box.json",
      "tags": ["startup", "team"]
    }
  ]
}
...
```bash
pnpm crewdeck store list                    # browse available modules and templates
pnpm crewdeck store install <module-id>     # install a module
pnpm crewdeck store import <template-id>    # import a company template
pnpm crewdeck store export                  # export current company as template
```
...
| **Multi-tenant** | Separate companies/orgs within one Crewdeck instance | `server:started` |
...
4. **Module config** — `crewdeck.config.json` with per-module config, env var interpolation
...
10. **`@crewdeck/module-sdk`** — TypeScript types for `ModuleAPI`, `HookEvent`, `HookHandler`, manifest schema
