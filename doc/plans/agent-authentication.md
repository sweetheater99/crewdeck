...
Agents need API keys to authenticate with Crewdeck. The current approach
...
**Trust model:** The adapter process runs on the same machine as the Crewdeck
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** Crewdeck generates a token and passes it directly to the agent
...
`CREWDECK_API_URL`, `CREWDECK_AGENT_ID`, `CREWDECK_COMPANY_ID`. We need to
add a `CREWDECK_API_KEY` (JWT) to the set of injected env vars.
...
**Approach:** Similar to `claude setup-token` -- the developer runs a Crewdeck CLI
...
```
crewdeck auth login
# Opens browser -> user confirms -> token stored at ~/.crewdeck/credentials
```
...
managed by the Crewdeck server itself.
...
1. A company admin (user or agent) generates an **invite URL** from Crewdeck.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The Crewdeck SKILL.md (or a link to it)
   - What information Crewdeck needs from the agent (e.g. webhook URL, adapter
...
5. Crewdeck stores the pending registration.
...
7. On approval, Crewdeck provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** Crewdeck issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "skillUrl": "https://app.crewdeck.ing/skills/crewdeck/SKILL.md",
    "requiredFields": {
      "name": "Your display name",
      "adapterType": "How Crewdeck should send you heartbeats",
...
1. Generate a Crewdeck invite link for the company.
...
4. A Crewdeck company member approves the new agent.
5. Crewdeck begins sending heartbeats to the OpenClaw webhook endpoint.
...
| Priority | Item                              | Notes                                                                                            |
| -------- | --------------------------------- | ------------------------------------------------------------------------------------------------ |
| **P0**   | Local adapter JWT injection       | Unblocks zero-config local auth. Mint a JWT per heartbeat, pass as `CREWDECK_API_KEY`.          |
...
| **P3**   | CLI auth flow                     | `crewdeck auth login` for developer-managed remote agents.                                      |
