...
- New config/env settings:
  - `CREWDECK_AGENT_JWT_SECRET`
  - `CREWDECK_AGENT_JWT_TTL_SECONDS` (default: `172800`)
  - `CREWDECK_AGENT_JWT_ISSUER` (default: `crewdeck`)
  - `CREWDECK_AGENT_JWT_AUDIENCE` (default: `crewdeck-api`)
...
1. In:
   ...
   inject `CREWDECK_API_KEY` from context token.

- Preserve existing behavior for explicit user-defined env vars in `adapterConfig.env`:
  - if user already sets `CREWDECK_API_KEY`, do not overwrite it.
- Continue injecting:
  - `CREWDECK_AGENT_ID`
  - `CREWDECK_COMPANY_ID`
  - `CREWDECK_API_URL`
