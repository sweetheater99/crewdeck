...
Add a single storage subsystem for Crewdeck that supports:
...
- API serves attachment bytes through authenticated Crewdeck endpoints.
...
- Default local path is under instance root: `~/.crewdeck/instances/<instanceId>/data/storage`.
...
- `crewdeck onboard` writes a valid `storage` config block by default.
- `crewdeck configure --section storage` can switch between local and s3 modes.
...
- `crewdeck doctor` reports actionable storage status.
