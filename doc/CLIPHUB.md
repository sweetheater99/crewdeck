...
ClipHub is the public registry where people share, discover, and download Crewdeck company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to Crewdeck what a package registry is to a programming language. Crewdeck already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in Crewdeck — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own Crewdeck instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the Crewdeck spec. It contains:
...
**Install (fresh start):**
```
crewdeck install cliphub:<publisher>/<company-slug>
```
Downloads the template and creates a new company in your local Crewdeck instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within Crewdeck, export your company as a template, then publish:

```
crewdeck export --template my-company
crewdeck publish cliphub my-company
```
...
ClipHub is a **separate service** from Crewdeck itself. Crewdeck is self-hosted; ClipHub is a hosted registry that Crewdeck instances talk to.
...
| **Crewdeck CLI** | `crewdeck install`, `crewdeck publish`, `crewdeck cliphub sync` — built into Crewdeck |
| **Crewdeck UI** | "Browse ClipHub" panel in the Crewdeck web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with Crewdeck) |
| Backend | TypeScript + Hono (consistent with Crewdeck) |
| Database | PostgreSQL |
...
4. Run `crewdeck install cliphub:acme/lean-saas-shop`
5. Crewdeck creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in Crewdeck until it works well
2. Export: `crewdeck export --template my-agency`
3. Publish: `crewdeck publish cliphub my-agency`
...
3. Install just that agent: `crewdeck install cliphub:acme/senior-python-eng --agent`
4. Assign it to a manager in your existing company
5. Done

---

## Relationship to Crewdeck

ClipHub is **not required** to use Crewdeck. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to Crewdeck what a package registry is to a language runtime: optional, but transformative.
...
- [ ] `crewdeck install cliphub:<publisher>/<slug>` CLI command
...
- [ ] "Browse ClipHub" panel in Crewdeck web UI
- [ ] `crewdeck cliphub sync` for bulk publishing
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with Crewdeck's own philosophy)
