...
This spec defines how Crewdeck actually runs agents while staying runtime-agnostic.
...
1. Crewdeck is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, Crewdeck manages lifecycle, and on exit Crewdeck parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `crewdeck.skill` (shared Crewdeck skill text block)
...
2. Preferred transport is env vars (`CREWDECK_*`) injected at runtime.
